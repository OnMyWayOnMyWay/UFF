...
local STAT_DEFINITIONS = {
	Passing = {
		stats = {"Attempts", "Average", "Completion %", "Completions", "Interceptions", "Longest", "Rating", "Touchdowns", "Yards"},
		calculated = {"Rating", "Completion %", "Average"}
	},
	Rushing = {
		stats = {"20+ Yards", "Attempts", "Longest", "Touchdowns", "Yards", "Yards Per Carry"},
		calculated = {"Yards Per Carry"}
	},
	Receiving = {
		stats = {"Drops", "Longest", "Receptions", "Targets", "Touchdowns", "Yards"},
		calculated = {}
	},
	Defense = {
		stats = {"Interceptions", "Pass Deflections", "Sacks", "Safeties", "Tackles", "Tackles For Loss"},
		calculated = {}
	}
}
...
local SUBMISSION_STATS = {
	passing = {"Attempts", "Average", "Completion %", "Completions", "Interceptions", "Longest", "Rating", "Touchdowns", "Yards"},
	rushing = {"20+ Yards", "Attempts", "Longest", "Touchdowns", "Yards", "Yards Per Carry"},
	receiving = {"Drops", "Longest", "Receptions", "Targets", "Touchdowns", "Yards"},
	defense = {"Interceptions", "Pass Deflections", "Sacks", "Safeties", "Tackles", "Tackles For Loss"}
}
...
local function getPlayerTeam(userId)
	-- Check current team if player is in game
	local player = Players:GetPlayerByUserId(userId)
	if player and player.Team then
		local teamName = player.Team.Name
		local currentTeamKey = nil

		-- Determine if it's Home or Away based on team names (case-insensitive comparison)
		...
		-- Update team value in all folders if current team is determined
	...
	-- Fallback to stored value if player not in game or team not recognized
	for category, _ in pairs(SUBMISSION_STATS) do
		local folder = getPlayerFolder(category, userId)
		if folder then
			local teamValue = folder:FindFirstChild("Team")
...
local function ensurePlayerFolder(category, userId)
	...
	-- getPlayerTeam will update team value if player's team changed
...
local function calculatePassingStats(userId)
	...
	-- Completion Percentage (1 decimal place)
	local compPct = math.floor((completions / attempts) * 1000 + 0.5) / 10
...
local function gatherPlayerStatsForAPI(userId, category)
	...
	if categoryKey == "passing" then
		-- SCKED is required for passing stats in API
...
local function syncSubmissionData(category, userId)
	...
	-- Get current team (this will update stored values if player switched teams)
	local teamKey = getPlayerTeam(userId)
...
function StatsManager:AddStatLine(category, userId, statKey, value)
	...
	if not statInstance or not statInstance:IsA("ValueBase") then
		-- Create missing stat on the fly to avoid lost stats
		local newStat
		if statKey == "Yards" or (value and typeof(value) == "number" and value % 1 ~= 0) then
			newStat = Instance.new("NumberValue")
		else
			newStat = Instance.new("IntValue")
		end
		newStat.Name = statKey
		newStat.Value = 0
		newStat.Parent = folder
		statInstance = newStat
	end
...
--- Refresh all player team values based on current team assignments
function StatsManager:RefreshAllPlayerTeams()
	-- Go through all categories and refresh team values for all players
	for category, _ in pairs(SUBMISSION_STATS) do
		local categoryFolder = getCategoryFolder(category)
		if categoryFolder then
			for _, playerFolder in ipairs(categoryFolder:GetChildren()) do
				...
				if userId then
					-- This will update team values based on current team assignments
					getPlayerTeam(userId)
				end
			end
		end
...
function StatsManager:SetTeamInfo(team, name, color)
	...
	-- If team name changed, refresh all player team values
	...
	for category, _ in pairs(SUBMISSION_STATS) do
		...
		if categoryFolder then
			for _, playerFolder in ipairs(categoryFolder:GetChildren()) do
				local userId = tonumber(playerFolder.Name)
				if userId then
					-- This will update team values if player switched teams
					syncSubmissionData(category, userId)
				end
			end
		end
